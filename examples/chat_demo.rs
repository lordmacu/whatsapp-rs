...
async fn main() -> Result<()> {
    ...
    let reply_waiter = chat.listen_for_reply(Duration::from_secs(60));
    println!("→ reply listener armed (60s window)");
    ...
    println!("→ awaiting reply (up to 60s total from listener start)…");
