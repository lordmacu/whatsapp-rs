...
pub fn x3dh_sender(our_identity: &KeyPair, bundle: &PreKeyBundle) -> X3DHResult {
    ...
    let root_key = kdf_x3dh(&ikm);
    X3DHResult { root_key, ephemeral_key: ek_a }
}
...
fn kdf_x3dh(ikm: &[u8]) -> [u8; 32] {
    ...
    // It splits output into rootKey(0..32) + chainKey(32..64). We only
    // return the root key here; the chain key half is currently discarded
    // (the ratchet rebuilds its send_chain on first encrypt via kdf_rk).
